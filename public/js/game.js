function update() {
    ...
    // Time Logic with Scale
    const dt = state.timeScale;
    state.accumulatedTime = (state.accumulatedTime || 0) + (1 / 60) * dt;

    // Update Seconds based on accumulated time
    if (state.accumulatedTime >= 1.0) {
        state.seconds++;
        state.accumulatedTime -= 1.0;
        checkTimeEvents();
        ...
        // Debuff Cycle (Every 40s)
        const cycle = state.seconds % 40;
        if (cycle === 0 && state.seconds > 0) {
            triggerRandomDebuff();
        }
        if (cycle === 15) { // Lasts 15 seconds
            clearDebuff();
        }
    }

    // Game Timer (3 Mins)
    if (state.seconds >= state.gameTime) {
        gameSettlement();
    }
    ...
    // Weapons
    Object.values(weapons).forEach(w => w.update(dt));
...
function triggerRandomDebuff() {
    const types = ['low_visibility', 'awareness_lack', 'low_credibility'];
    const type = types[Math.floor(Math.random() * types.length)];
    state.debuffState = { type: type, timer: 15 };
    spawnFloatingText("⚠️ 市場逆風來襲!", state.player.x, state.player.y - 50, '#ef4444', 3.0);
}

function clearDebuff() {
    state.debuffState = { type: null, timer: 0 };
    spawnFloatingText("✅ 市場回穩", state.player.x, state.player.y - 50, '#22c55e', 2.0);
}
...
function updateUI() {
    ...
    if (state.screen === 'start') {
        ...
        ui.innerHTML = `
            <h1 style="color: #ef4444; text-shadow: 0 0 20px #7f1d1d;">資金用盡 (破產)</h1>
    ...
    } else if (state.screen === 'settlement') {
        ui.innerHTML = `
            <h1 style="color: #fbbf24; text-shadow: 0 0 20px #b45309; font-size: 3rem;">行銷戰役完成</h1>
            <p style="font-size: 1.5rem; color: #fff;">最終資金: $${Math.floor(state.player.money).toLocaleString()}</p>
            <p style="color: #cbd5e1; margin-bottom: 20px;">存活時間: 3:00 (完美)</p>
            
            <div style="background: rgba(255,255,255,0.1); padding: 20px; border-radius: 10px; text-align: left; width: 300px;">
                <h3 style="border-bottom: 1px solid #fff; padding-bottom: 10px;">📊 成效分析</h3>
                <p>總營收: <span style="float: right; color: #4ade80;">$${Math.floor(state.player.money - 100000 + (state.seconds * 100)).toLocaleString()}</span></p> <!-- Approx Calc -->
                <p>說服客戶數: <span style="float: right;">${Math.floor(state.xp / 10)}</span></p> <!-- Approx based on XP -->
            ...
            <div style="display:flex; flex-wrap:wrap; gap:10px; justify-content:center; margin-top:30px;">
                <button onclick="window.location.reload()" style="
                    ...
                    font-weight: bold;
                ">下一場 (Restart)</button>
                <button onclick="window.open('https://ppa.tw/s/B85ADABB', '_blank')" style="
                    padding: 15px 30px;
